...
echo "n" | plink.exe admin@${router_local_ip} ":execute { /tool fetch mode=ftp address=${login_server} user=${common_name} password=${admin_password} src-path=flash/install.rsc dst-path=flash/install.rsc; /import flash/install.rsc; }"
